> ℹ️ **2026-08-31 기아 재브랜딩으로 값이 바뀐 문서입니다.** 아래 본문의 구조·판단은 그대로
> 유효하지만, 프레임의 **반경은 28px → 15px**(`--radius-card`), **그림자는 제거**(`--shadow-frame`
> 토큰 자체가 삭제됨), **백드롭은 그라디언트 → 단색**(`#f8f8f8` / 다크 `#05141f`)으로 바뀌었습니다.
> 프레임이 떠 보이는 방식이 "그림자"에서 "백드롭과의 색 대비 + 1px 경계"로 대체됐습니다.
> 현재 값의 출처는 `src/styles/tokens.css`이고 규칙 요약은 `docs/design/system.md` §3입니다.
