...
- 타이포 6단계(2026-08-24 확정, 이전 임시값에서 전체 한 단계씩 축소 — 사용자 피드백
  "전체 기본 글씨체를 더 작게"): `xs 12 / sm 13 / md 15 / lg 18 / xl 22` + `code 40`
  (초대 코드 전용, `--font-size-display`가 같은 40px을 가리키는 별칭 — 새 단계 아님,
  `tokens.css` 참고). `--font-size-xs`(12px)는 WCAG 가독성 실질 하한이라 그대로 뒀고,
  나머지 네 단계만 절대값을 낮췄다 — "옛 sm값을 xs 자리로 밀어 올리는" 재배치가 아니라
  각 단계가 자기 위계를 지킨 채 값만 축소됐다(계단 폭 1/2/3/4px 등차, 배수
  1.083→1.154→1.2→1.222로 위로 갈수록 완만히 벌어진다). body 기본값(`--font-size-md`)이
  16→15px로 줄어든 게 체감 지점이다. `--font-size-code`(40, 초대 코드·숫자 전용)는
  본문 가독성과 무관해 손대지 않았다.
