...
### 노선 색 파생 배경 — 프로필 추천 칩 (2026-08-24, 사용자 피드백)

`ProfileScreen`의 "다음에 가볼만한 역" 칩(`profile.module.css` `.chip`)은 배경을
`--color-surface` 고정에서 **소속 노선 색의 파스텔톤**으로 바꿨다. 새 토큰을 만들지 않고
`line-map.module.css` §F-03(역 심볼 채움 농도)의 `color-mix` 방법론을 재사용했다 — 비율만
배경 용도에 맞게 새로 계산했다(그 파일의 65%는 아이콘 채움용이라 배경엔 너무 진하다).

- **컴포넌트 계약**: `.chip` 엘리먼트가 인라인 커스텀 프로퍼티 `--chip-line-color`를
  받는다고 가정한다. 값은 `StationPicker.tsx` 배지 점과 같은 폴백 체인이다:
  `var(--{rec.lines[0].colorToken}, var(--line-default))`. `rec.lines.length === 0`인
  드문 경우엔 이 스타일 자체를 꽂지 않는다 — CSS의 `color-mix(..., var(--chip-line-color,
  var(--color-surface)) N%, var(--color-surface))`가 자기 자신으로 폴백해 "서피스 N% +
  서피스 (100-N)%" = 서피스가 되므로 별도 규칙 없이 지금 배경 그대로 떨어진다.
- **비율**: 기본 12%, hover 20%. `@supports (background: color-mix(...))`로 감싸고,
  미지원 브라우저(iOS Safari 16.0~16.1)는 가드 밖의 미믹스 `--color-surface` 그대로 쓴다.
- **대비 계산** (현재 유일한 실색 노선인 `--line-2`, 라이트 `#009944` / 다크 `#2dc46f` 기준.
  그레이스케일 상대휘도, sRGB 성분별 선형 혼합):

  | 상태 | 비율 | `.chipLine`(--color-text-muted, 12px) 대비 | 기준 | `.chipName`(--color-text) 대비 |
  |---|---|---|---|---|
  | 기본 | 12% | 라이트 6.76:1 / 다크 5.86:1 | 4.5:1 | 라이트 18.1:1 / 다크 13.9:1 |
  | hover | 20% | 라이트 6.12:1 / 다크 **4.97:1** | 4.5:1 | 라이트 6.1:1+ (여유 충분) |

  hover(다크, 20%)가 가장 타이트한 지점이다(여유 0.47). 25%부터는 다크에서 `--line-2` 기준
  이미 4.45:1로 미달해 kept 아래로 내렸다. `--color-text-muted`는 `.chipLine`(12px, xs)에
  쓰이므로 큰 텍스트 3:1이 아니라 본문 4.5:1 기준을 적용했다.
- **미확정 노선 색 경고**: 지금 `--line-2` 외 노선은 `--line-default`(그레이)로 떨어진다
  (`StationPicker.tsx` 주석 참고, "노선 색이 없는 노선은 대비를 보장할 수 없다"). 새 노선
  색 토큰이 추가되면(특히 다크에서 `--line-2`보다 밝고 채도 높은 색) 이 12%/20% 비율의
  대비를 반드시 재계산해야 한다 — 여유가 0.47밖에 없어 쉽게 깨진다.
- 테두리(`--color-border-strong`)는 손대지 않았다 — 배경만 바뀌었을 뿐 칩의 경계 신호는
  그대로 유지한다.
...
- 타이포 6단계. 두 차례 축소 이력이 있다.
  - **2026-08-24 1차** (이전 임시값에서 전체 한 단계씩 축소 — 사용자 피드백 "전체 기본
    글씨체를 더 작게"): `xs 12 / sm 13 / md 15 / lg 18 / xl 22` + `code 40`.
    `--font-size-xs`(12px)는 WCAG 가독성 실질 하한이라 그대로 뒀고, 나머지 네 단계만
    절대값을 낮췄다 — "옛 sm값을 xs 자리로 밀어 올리는" 재배치가 아니라 각 단계가 자기
    위계를 지킨 채 값만 축소됐다(계단 폭 1/2/3/4px 등차, 배수 1.083→1.154→1.2→1.222로
    위로 갈수록 완만히 벌어진다). body 기본값(`--font-size-md`)이 16→15px로 줄어든 게
    체감 지점이었다.
  - **2026-08-24 2차** (실사용 피드백 "폰트사이즈 더 작게" — 1차로도 부족하다는 재확인):
    `xs 12 / sm 13 / md 14 / lg 16 / xl 19` + `code 40`. `--font-size-xs`(12px)는 이번에도
    하한선 그대로다 — 사용자가 "12px 밑으로는 내리지 마라"고 명시적으로 재확인했고, Noto
    Sans KR 국문 자모 획 밀도상 12px 미만은 저해상도·흐린 화면에서 자모 구분이 무너지기
    시작하는 실질 하한이라는 1차 판단이 재검증 후에도 유지된다. 대신 나머지 네 단계를
    xs 쪽으로 더 바짝 당겼다: 계단 폭이 1/2/3/4px(1차) → 1/1/2/3px(2차)로 좁아졌다 —
    위로 갈수록 폭이 벌어지는 방향성(등차 계열)은 유지해 단계 간 위계는 여전히 구분된다.
    body 기본값(`--font-size-md`)이 15→14px로 한 번 더 줄어든 게 이번 체감 지점이다.
    14px는 전문 UI에서 통용되는 본문 크기 하한권이라 xs(12px)와 2px 차이를 유지한다.
    sm(13)·md(14)는 1px 차이로 전 단계 중 가장 촘촘하지만 용도(라벨류 vs 본문)와
    굵기(weight) 대비로 구분된다.
  (`code 40`은 초대 코드 전용, `--font-size-display`가 같은 40px을 가리키는 별칭 — 새
  단계 아님, `tokens.css` 참고. `--font-size-code`는 본문 가독성과 무관해 두 차례 모두
  손대지 않았다.)
...
### z-index 3단계 (2026-08-26 추가)

| 토큰 | 값 | 쓰는 곳 | 쓰면 안 되는 곳 |
|---|---|---|---|
| `--z-canvas` | 0 | 노선도 SVG·카카오맵처럼 **남의 DOM이 들어오는 캔버스 컨테이너**. 0을 명시해 스택 문맥을 만드는 게 목적이다 | 일반 콘텐츠(값이 없어도 되는 것에 0을 붙이면 스택 문맥이 늘어난다) |
| `--z-canvas-control` | 10 | 캔버스 **위**에 얹히는 컨트롤·오버레이(`.zoomControls`=줌/전체 핀 보기, 지도 빈 상태 안내) | 캔버스 밖 요소 |
...
- `.mapDiv`에 `--z-canvas`(0)를 **명시**해 SDK 내부 z-index를 그 안에 가둔다.
- `.canvasWrap`에 `isolation: isolate`를 줘 캔버스 영역 전체가 앱 크롬·다이얼로그와 같은
  평면에서 경쟁하지 않게 한다.
- 검증은 눈이 아니라 `document.elementFromPoint(버튼 중앙)`으로 한다. 수정 전에는 정보창
  DIV가, 수정 후에는 버튼이 반환된다(2026-08-26 실 SDK 하니스로 양쪽 다 실측).
...
- **타이포 크기(`--font-size-*`)는 2026-08-24로 확정됐다** (§4). 행간(`--line-height-*`)은
