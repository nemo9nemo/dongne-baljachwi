...
| `--color-surface` | `#ffffff` | `#05141f` | 카드, 탭바, 다이얼로그 | 라이트에선 bg와 같은 값이다 — 카드를 카드로 만드는 건 **테두리+반경**이지 이 색이 아니다. **입력의 hover/focus 바닥으로도 더 이상 쓰지 않는다**(2026-09-02) |
| `--color-surface-muted` | `#f8f8f8` | `#14232e` | 섹션 구분, hover 바닥, **로딩 스켈레톤**, 배지/칩 바탕 | 텍스트 위 오버레이 |
| `--color-field` | `#f8f8f8` | `#14232e` | 브라우저 기본 크롬이 있는 폼 컨트롤(`<select>`, 파일 첨부 드롭존, 추천 칩) 바닥 | **`<Input>`/`<Textarea>`** — 2026-09-02 사용자 요청으로 이 둘의 채움을 걷어냈다(테두리만). 버튼·카드 |
...
- **대비 재계산 (2026-08-31, 새 팔레트 × MVP 24개 노선색 전부):** 아래는 24색 중 **최저값**이다.
  ...
  한 색만 보고 올리지 않는다.
- 칩 형태는 바뀌었다: 알약(999px) → `--radius-control`(2026-09-02부터 10px), 좌우 패딩 24px.
...
- **반경**: 역할 토큰 두 개가 전부다.
  - `--radius-control: 10px` — 버튼·입력·칩·태그·배너·메뉴·토스트·FAB·썸네일·섹션.
    **2026-09-02에 0px → 10px.** 기아 원안은 완전 각짐이었으나 사용자가 실사용 후 부드러운
    모서리를 요청했다. 값 하나만 바꿔 전 컨트롤에 전파된다(리터럴 반경이 없기 때문에 가능한 것).
  - `--radius-card: 15px` — 카드, 다이얼로그 3종, 앱 프레임, 카드 자리를 대신하는 스켈레톤.
    control과 5px 차이라 위계(컨트롤 < 카드)는 유지된다
...
- `--radius`는 `--radius-control`(10px)이다. shadcn 기본 테마의 `calc(var(--radius) - 2px)`
  파생 공식을 쓰지 않는다 — 이 시스템의 반경은 계산식이 아니라 두 개의 역할 토큰이다.
- `--secondary`와 `--accent`가 같은 중립 톤을 가리키는 것은 의도다. 이 브랜드에 "악센트 색"은 없다.
- Tailwind 매핑(`tailwind.config.ts`): `rounded-sm/DEFAULT/md` → control(10px),
