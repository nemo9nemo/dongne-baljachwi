> ⚠️ **폐기된 이전 방향입니다 (2026-08-31).**
> 사용자가 기아 웹사이트 디자인 시스템 기반 **전면 재브랜딩을 확정**해, 이 문서가 기술하는
> 시각 방향(흑백 그레이스케일 + Resend식 반투명 헤어라인 + 12px 반경 + 그림자 위계)은 더 이상
> 유효하지 않습니다. **현재 기준은 `docs/design/kia-reference-redesign.md`와
> `docs/design/system.md`입니다.** 이 문서는 히스토리 보존용으로만 남깁니다 — 여기 적힌 토큰
> 값·반경·그림자·굵기 규칙을 근거로 코드를 고치지 마세요.
>
> 다만 §4의 **shadcn/ui 변수 별칭 전략**(`--background` 등이 `--color-*`를 가리키는 구조)과
> §9의 컴포넌트 마이그레이션 기록은 재브랜딩 이후에도 그대로 살아 있습니다 — 바뀐 것은
> 그 별칭들이 가리키는 **값**뿐입니다.

---

# Resend 레퍼런스 재구성 — 판단 기록 (2026-08-21, 폐기됨)
