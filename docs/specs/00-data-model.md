...
| 함수 | 용도 | 정의 위치 |
|---|---|---|
| `couple_record_months(p_tag_norm text default null)` | 타임라인 연·월 인덱스(기록이 있는 달과 건수) | `13-timeline-time-jump.md` §4.1이 정본 |

- 뷰가 아니라 함수인 이유(태그 축을 파라미터로 받기 위해)는 `13` §4.1에 있다.
- **`SECURITY INVOKER`여야 한다.** D-12가 뷰에 요구한 것과 같은 이유이며, DEFINER로
  만들면 전 커플의 기록 분포가 노출된다. 함수는 뷰와 달리 기본값이 INVOKER지만,
  실수로 DEFINER를 붙이지 않도록 명시적으로 적어 둔다.
...
> ✅ **결정 완료 (2026-08-25) — 커플 표시명**: (a) 두 사람 이름 나열로 확정한다.
> `couples.title` 같은 커플 공동 별칭 컬럼은 **추가하지 않는다**. 표시 형식은
> `{owner.display_name} · {partner.display_name}` (가운데 점 구분, `couple_members.role`
> 기준 owner 먼저 — 이미 존재하는 컬럼이라 정렬 축을 새로 만들 필요가 없다). 상대 미합류
> 상태(멤버 1명)의 표시는 이미 `01-auth-couple-link.md` §2.1/`09-couple-profile.md` §2.2가
> "상대 초대 대기 중"으로 별도 정의하고 있어 이 결정과 충돌하지 않는다.
>
> **근거**: 별칭(b)을 택하면 온보딩에 입력 스텝이 하나 늘어나는데, PRD §5.2가 명시한
> "입력 마찰 최소화"라는 이 서비스의 설계 원칙과 정면으로 부딪힌다. 또한 커플이 나중에
> 별칭을 바꾸고 싶어할 때 그 변경이 과거 기록에 어떻게 반영돼야 하는지(소급 여부)라는
> 새로운 결정을 하나 더 만든다. 두 사람 이름은 이미 `profiles.display_name`으로 존재하는
> 데이터라 추가 스키마·추가 입력 없이 바로 조합할 수 있어 구현 비용도 0이다. "우리만의
> 애칭"이라는 정서적 가치는 실제로 있을 수 있지만, PRD 어디에도 요구된 적 없는 기능이라
> `CLAUDE.md`의 "PRD에 없는 기능을 슬쩍 추가하지 않는다" 원칙에 따라 만들지 않는다.
> `09-couple-profile.md` §9의 동일 항목(구 41번)도 이 결정으로 함께 해소한다.

> ✅ **결정 완료 (2026-08-25) — 해제 후 데이터 보존 기간 30일**: 그대로 유지한다(변경 없음).
>
> **근거**: `01-auth-couple-link.md` F-22가 이미 "해제 취소(복구)는 제공하지 않는다"로
> 확정돼 있어, 이 30일은 **사용자 대면 복구 기능**이 아니라 (1) 개발자가 수동으로 개입할
> 수 있는 최소한의 유예 기간(예: 실수로 오조작한 사용자의 고객 지원 요청 대응), (2) 국내
> 다수 서비스가 회원 탈퇴/데이터 삭제 유예 기간으로 채택하는 관례적 길이(30일)라는 두 가지
> 근거로 유지한다. 1년처럼 더 길게 늘리는 것은 "해제했는데도 데이터가 오래 남아있다"는
> 프라이버시 우려만 키우고, 즉시 삭제는 F-23(2단계 확인)에도 불구한 오조작을 되돌릴 방법을
> 완전히 없앤다 — 30일이 둘 사이의 합리적인 중간값이다.
>
> **해제 전 일괄 내보내기(백업) 기능**은 만들지 않는다. PRD에 없고, `10-record-card-image.md`의
> 이미지 저장 기능(기록 단위)으로 사용자가 원하는 기록을 개별적으로 미리 저장해둘 수 있는
> 대체 경로가 이미 존재한다. 일괄 내보내기(예: ZIP 다운로드)는 별도 요구가 확인되면 그때
> 검토한다.

> ✅ **결정 완료 (2026-08-25) — 계정 삭제(탈퇴)**: **이번 범위에서 셀프서비스 탈퇴 플로우를
> 만들지 않는다.** PRD 어디에도 계정 탈퇴 요구가 없고, 이 서비스의 개인정보 접근 경계는
> 사실상 "커플 단위"라 커플 해제(F-19~F-23) 자체가 이미 상대방에 대한 접근을 끊는
> 주된 수단으로 기능한다. `records.author_id NOT NULL` 제약은 그대로 둔다 — 셀프서비스
> 탈퇴 경로가 없으므로 탈퇴 시 FK 충돌 시나리오 자체가 발생하지 않는다.
>
> 다만 개인정보보호 관점에서 "탈퇴 요청"이 완전히 0건일 거라 가정하는 건 안전하지 않으므로,
> **운영 절차(코드 아님)로만 대응**하기로 한다: 사용자가 탈퇴를 요청하면 개발자가 수동으로
> ① 먼저 커플을 해제시키고 ② `auth.users`를 Supabase 콘솔에서 직접 삭제한다(CASCADE로
> `profiles`도 함께 삭제됨). 이 절차는 셀프서비스가 아니므로 화면·API를 만들지 않는다.
> 사용자 규모가 커져 수동 대응이 감당 안 되는 시점이 오면 이 결정을 재검토한다 — 그 전까지는
> `CLAUDE.md`의 "나중에 쓸 것 같아서 만드는 기능 금지" 원칙에 따라 만들지 않는 것이 맞다.
...
- 2026-08-25 §9 미결정 3건 확정(planner) — 커플 표시명은 두 사람 이름 나열(`{owner} · {partner}`, 별칭 컬럼 미추가)로, 해제 후 보존기간은 30일 그대로 유지(일괄 내보내기는 미구현), 계정 탈퇴는 셀프서비스 플로우를 만들지 않고 수동 운영 절차로만 대응하기로 확정.
- 2026-08-26 §4.9에 집계 함수 `couple_record_months()` 추가(정본은 `13-timeline-time-jump.md` §4.1, `SECURITY INVOKER` 필수). 테이블·컬럼·RLS 정책 변경 없음. 신규 스펙 `11`(오프라인 사진 캐시)은 클라이언트 로컬 저장소만 쓰므로 이 문서에 반영할 서버 계약이 없다.
