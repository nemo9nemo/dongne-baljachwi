...
const EXCLUDE_STATION_CODES = new Set([
  ...
  // 아래는 위 항목들과 성격이 다르다 — "수도권 밖 실재 지역"이 아니라 **DB 좌표 자체가
  // 잘못된 데이터 오류**다. 2026-08-24 경의중앙선 배치 중 viewBox 폭이 3882→15245.6로
  // 폭증해 역추적한 결과, `stations.lat/lng`가 (36.96, 129.09) — 경상북도 포항/영덕
  // 인근으로 찍혀 있었다(실제 양원역은 경기 가평, 인접역 백양리·강촌과 비슷한 위치여야
  // 한다). `region_code`도 '11'(서울)로 되어 있어 위경도와도 자기모순이다. 서울시청
  // 기준 100km 초과 MVP 역을 전수 스캔한 결과 이 역이 유일한 이상치였다(2026-08-24
  // 확인) — `02-station-master.md`가 소유한 원천 데이터 버그이므로 이 스크립트가 고칠
  // 수 없다. 재적재(`stations:load`)로 고쳐질 때까지 좌표 작업에서만 제외한다.
  'S-I4108-1204', // 양원역 — 좌표 데이터 오류 (경북 좌표로 잘못 적재됨), 원천 수정 필요
]);
